...
async function buyAndSellWHILE_CANDLE(){ // 변동성전략을 이용한 구매하는 함수
    while(true){
        await sleep(1000*10)
        if(Object.keys(_CANDLES).length < _CANDLES_MARKETS.length) continue;
        for (var i in _CANDLES){
            ...
            else if(close_price > price){ // 현재 가격이 떨어진다면 그 즉시 판매 상승장에서 하락장 조짐이 보이면 판매 //내가 느끼기엔 이게 정답이다 
...
async function main(){
    //변동성돌파 식 캔들은 지금시간보다 전의 캔들을기준으로삼는다
        ...
        await buyAndSellWHILE_CANDLE() //사고팔고.
        console.log("rsi지표 프로그램을 불러옵니다.")
        rsi()
        // sellAll()
        // console.log(_CANDLES[0])
        // body = await API_buyImmediate("KRW-DKA", 20000000);   // 정상 구매
