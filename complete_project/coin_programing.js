...
async function get_candles(market){ // 캔들정보를 가져오는 함수
    url = 'https://api.upbit.com/v1/candles/minutes/30?market='+market+'&count=2'
...
async function buyWHILE_CANDLE(){ // 변동성전략을 이용한 구매하는 함수
    while(true){
        ...
        for (var i in _CANDLES){
            ...
            target_price = close_price+(high_price-low_price)*0.5//목표가  //원래는 0.5지만 단타를 위해 수정
            // console.log(_CANDLES[i][0].market+"의 목표가를 갱신했습니다.") // 목표가 제대로 되었는지 확인
            console.log(market+"의 목표가는 :"+target_price)
            console.log("현재 가격은 :"+price)
...
async function SellWHILE_CANDLE(){
    ...
    while(true){
        ...
        for (var i in _CANDLES){
            ...
            // console.log(price+"현재가")
            // console.log(close_price+"이전종가")
            // if(close_price*0.99> price){ // 현재 가격이 떨어진다면 그 즉시 판매 상승장에서 하락장 조짐이 보이면 판매 //내가 느끼기엔 이게 정답이다 
        ...
        // else if (low_price>price){//코인 단타칠때 손절라인을 빠르게 잡아서 손해를 보지않는다
            ...
            for(var i in balance){ //현재 내 지갑에 있는 정보를 가져온다.
                if("KRW-" + balance[i].currency == market){
                    ...
                    // if(balance[i].avg_buy_price < price){   
                        if(high_price*0.98>price){//전고가 -2퍼가 되는순간 팔린다
                            if(balance[i].avg_buy_price*1.00005 >price){//수수료를 포함한 가격보다 낮아질시 바로판매
                            if(volume > 0){
                                await API_sellImmediate(market, volume);
                            
                                console.log(market+"을 판매했습니다.@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@")
