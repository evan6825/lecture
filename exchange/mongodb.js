...
const dbconfig = require("./config/database.json"); //파일을 만들어야함
const accounts_name = require("./config/config.json");
...
function accounts_push(balance) {
  arr[0] = {
    currency: "KRW",
    balance: balance.toString(),
    timestamp: new Date().toLocaleString("en", { timeZone: "Asia/Seoul" }),
  };
  for (var i = 1; i < accounts_name.markets.length; i++) {
    arr[i] = {
      currency: accounts_name.markets[i],
      balance: "0.0",
      avg_buy_price: "0",
      timestamp: new Date().toLocaleString("en", { timeZone: "Asia/Seoul" }),
    };
...
function getSavedAccounts() {
  ...
  for (var i in _ALL_ACCOUNTS) {
    ...
    // let KRW_balance = _ALL_ACCOUNTS[i]._doc.accounts[0].balance; // KRW 잔액
    console.log(_ALL_ACCOUNTS[i]._doc, "==========");
    retJSON[accessKey] = {
      _id: accessKey,
      accessKey: accessKey,
      accounts: accounts,
...
// 최초의 돈을 지급할때 특정키에 밸런스를 추가
async function addAccount(accessKey, balance) {
  accounts_push(balance);
  console.log(arr);
...
async function initCreate() {
  init();
  testAccessKey = "TEST_ACCESSKEY3"; //나의 아이디가 드러감
  try {
    let balance = 1000000000; //초기 잔고
